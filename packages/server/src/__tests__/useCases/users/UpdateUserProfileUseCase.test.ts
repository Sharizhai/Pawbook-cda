...
describe.skip("Use case: ", () => {
    ...
    it("Should return an error if no user with this id is found", async () => {
        ...
        await expect(updateUserProfileUseCase.execute(nonExistentUserId)).rejects.toThrow("User not found");
    });

    it("Should throw an error if user id is missing", async () => {
        const emptyUserId = "";

        await expect(updateUserProfileUseCase.execute(emptyUserId)).rejects.toThrow("User ID is required");
    });
    ...
    it("Should throw an error if profileDescription is longer than 150 characters", async () => {
        ...
        await expect(updateUserProfileUseCase.execute(UnitUser.john.id, veryLongDescription)).rejects.toThrow("The description must not exceed 150 characters.");
    });
    ...
    it("Should update an existing user with valid datas", async () => {
        ...
        expect(result).toHaveProperty("email", validUpdateUserData.profileDescription);
    });
})